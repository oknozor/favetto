...
pub struct StateSourceConfig {
    ...
    /// an interactive TUI.
    pub headless: bool,
    /// Whether the launch owns a session created on a managed transport (e.g.
    /// opencode's managed server) rather than one generated by the caller. A
    /// headless run with a managed session can still be observed over that
    /// transport, unlike a plain stdout-only run.
