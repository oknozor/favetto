...
pub struct AgentContext {
    ...
    /// Whether `session_id` was created on a managed transport (opencode's
    /// server) rather than generated by the caller. The manager seeds it as the
    /// session's agent id even when the agent also probes its own output, so a
    /// concurrent interactive attach can target it immediately.
