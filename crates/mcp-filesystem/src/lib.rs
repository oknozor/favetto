...
//!
//! Tools: `list_dir`, `read_file`, `write_file`. All paths are resolved against
//! `--root` and may not escape it. Exposed both as a standalone binary and as a
//! library (`serve`) so the favetto daemon can run it in-process via a hidden
//! subcommand.
...
/// Run the server over stdio. `args` are the raw CLI args after the program name,
/// e.g. `["--root", "./workdir"]`.
pub async fn serve(args: Vec<String>) -> anyhow::Result<()> {
    let root = parse_root(args.into_iter());
